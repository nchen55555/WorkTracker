  ...
  useEffect(() => {
    if (isOpen) {
      if (initialTask) {
        ...
        setScheduledDate(initialTask.scheduledDate || getDefaultDate());
        // Check if the task has a time set
        const taskHasTime = !!(initialTask.startTime && initialTask.endTime);
        setHasTime(taskHasTime);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-[500px] mx-4 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-6 pb-6 space-y-5">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <button
                ...
                className={cn(
                  ...
                  hasTime ? "bg-[#FFDE59]" : "bg-[#E0E0D8]"
                )}
              >
                <span
                  className={cn(
                    "absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform",
                    hasTime && "translate-x-4"
                  )}
                />
              </button>
          ...
          {/* Time & Duration - only shown if hasTime */}
          {hasTime && (
            <div className="flex gap-3">
              ...
              <div className="flex-1">
                ...
                <select
                  ...
                  className="w-full px-4 py-3 bg-[#FFF8E1] border border-[#FFDE59] rounded-lg text-[#8B7355] font-medium appearance-none cursor-pointer focus:outline-none"
                  style={{
